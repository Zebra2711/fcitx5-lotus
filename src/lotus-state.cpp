...
namespace fcitx {
    ...
    bool LotusState::isAutofillCertain(const SurroundingText& s) {
        ...
        const size_t rangeStart = buffLen >= static_cast<size_t>(cursor) ? 0 : static_cast<size_t>(cursor) - buffLen;
    ...
    void LotusState::handleUinputMode(KeyEvent& keyEvent, KeySym currentSym, bool checkEmptyPreedit, int sleepTime) {
        ...
        int cursor = (ic_->surroundingText()).cursor();
        LOG("text=\""+escapeString(text)+"\"");
        LOG("cur="+std::to_string(cursor));
        LOG("real="+std::to_string(realtextLen));
